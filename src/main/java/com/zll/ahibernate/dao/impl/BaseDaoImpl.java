package com.zll.ahibernate.dao.impl;
...
import com.zll.ahibernate.annotation.Column;
import com.zll.ahibernate.annotation.Id;
import com.zll.ahibernate.annotation.Table;
import com.zll.ahibernate.dao.BaseDao;
import com.zll.ahibernate.util.TableHelper;
...
public class BaseDaoImpl<T> implements BaseDao<T> {
		...
		while (cursor.moveToNext()) {
			...
			for (Field field : this.allFields) {
				...
				if (field.isAnnotationPresent(Column.class)) {
					...
					} else if (Date.class == fieldType) {// 处理java.util.Date类型
						Date date = new Date();
	...
	/**
	 * @deprecated 有bug，使用update(T entity, int idValue)代替
	 */
	@Deprecated
	@Override
	 ...
	 */
	@Override
	public void update(T entity, int idValue) {
		SQLiteDatabase db = null;
		try {
			db = this.dbHelper.getWritableDatabase();
			ContentValues cv = new ContentValues();
			...
			cv.remove(this.idColumn);
			Log.d(TAG, "[update]: update " + this.tableName + " set " + sql
					+ " where " + where.replace("?", String.valueOf(idValue)));

			String[] whereValues = { String.valueOf(idValue)};
			db.update(this.tableName, cv, where, whereValues);
		} catch (Exception e) {
			Log.d(this.TAG, "[update] DB Exception.");
			e.printStackTrace();
		} finally {
